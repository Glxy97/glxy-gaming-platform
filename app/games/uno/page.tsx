...
import { UnoOnline } from '@/components/games/card/uno-online'
...
type GameMode = 'single' | 'online' | 'friends'
...
function UnoGameContent() {
  ...
  const [selectedMode, setSelectedMode] = useState<GameMode>(mode || 'single')
  ...
  if (gameStarted) {
    return (
      <div className="space-y-4">
        {/* Room Info Bar */}
        {roomId && (
          <Card className="bg-white/80 backdrop-blur border-red-300">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-yellow-50 to-green-50 p-4">
      <div className="container mx-auto max-w-4xl">
            ...
            >
              ...
              <CardContent className="text-center space-y-4">
                <p className="text-blue-700">
                  Play against intelligent AI bots. Perfect for practicing your UNO skills!
                </p>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {[
                      { key: 'easy', label: 'Easy', icon: '😊', desc: 'Relaxed gameplay' },
                      { key: 'medium', label: 'Medium', icon: '🤔', desc: 'Balanced challenge' },
                      { key: 'hard', label: 'Hard', icon: '😤', desc: 'Strategic AI' }
                    ].map((diff) => (
                      <Button
                        key={diff.key}
                ...
                <Button
                  ...
                  onClick={() => handleStartGame('single', selectedDifficulty)}
                >
            ...
            >
              ...
              <CardContent className="text-center space-y-4">
                <p className="text-purple-700">
                  Create a private room and invite your friends for an exciting UNO party!
                </p>
        ...
        {/* Features Showcase */}
        <motion.div
          className="bg-white/60 backdrop-blur rounded-xl p-6 border border-red-300"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.4 }}
        >
          <h3 className="text-2xl font-bold text-red-900 text-center mb-6">
            ✨ UNO Championship Features
          </h3>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            {[
              { icon: '🤖', title: 'Smart AI', desc: 'Intelligent bots with 3 difficulties' },
              { icon: '⚡', title: 'Real-time', desc: 'Live multiplayer synchronization' },
              { icon: '🎨', title: 'Animated', desc: 'Smooth card animations' },
              { icon: '🃏', title: 'Complete UNO', desc: 'All cards and rules implemented' }
            ].map((feature, index) => (
              <motion.div
                key={index}
              ...
              >
                ...
                <h4 className="font-bold text-red-900 mb-1">{feature.title}</h4>
                <p className="text-xs text-red-700">{feature.desc}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
