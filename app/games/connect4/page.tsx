...
import { Connect4_2025 } from '@/components/games/board/connect4-2025'
...
function Connect4GameContent() {
  ...
  const handleStartGame = (gameMode: GameMode, botDifficulty?: BotDifficulty) => {
    ...
    const newUrl = `/games/connect4?${newParams.toString()}`
  ...
  if (gameStarted) {
    return (
      <div className="space-y-4">
        {/* Room Info Bar */}
        {roomId && (
          <Card className="bg-white/80 backdrop-blur border-blue-300">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4">
      <div className="container mx-auto max-w-4xl">
            ...
            >
              ...
              <CardContent className="text-center space-y-4">
                <p className="text-blue-700">
                  Play against our intelligent Connect 4 AI. Perfect for practicing strategy!
                </p>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {[
                      { key: 'easy', label: 'Easy', icon: '😊', desc: 'Beginner level' },
                      { key: 'medium', label: 'Medium', icon: '🤔', desc: 'Balanced challenge' },
                      { key: 'hard', label: 'Hard', icon: '😤', desc: 'Expert strategy' }
                    ].map((diff) => (
                      <Button
                        key={diff.key}
            ...
            >
              ...
              <CardContent className="text-center space-y-4">
                <p className="text-purple-700">
                  Play against a friend on the same device. Take turns and enjoy classic Connect 4!
                </p>
        ...
        {/* Features Showcase */}
        <motion.div
          className="bg-white/60 backdrop-blur rounded-xl p-6 border border-blue-300"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.4 }}
        >
          <h3 className="text-2xl font-bold text-blue-900 text-center mb-6">
            ✨ Connect 4 Championship Features
          </h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            {[
              ...
              { icon: '🎨', title: 'Smooth UI', desc: 'Animated piece drops' },
              { icon: '🔴', title: 'Classic Rules', desc: 'Traditional Connect 4 gameplay' }
            ].map((feature, index) => (
              <motion.div
                key={index}
              ...
              >
                ...
                <p className="text-xs text-blue-700">{feature.desc}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-2xl">🔴 Loading Connect 4...</div>
      </div>
    }>
      <Connect4GameContent />
    </Suspense>
